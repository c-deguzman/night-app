module.exports = function(app){
	...
	app.post('/set_not_going', function(request, response) {
	    	...
	    	db.collection("goings", function (err, collection){
		        ...
		        collection.findOne({loc: raw_id, user: request.user._id, time: {$eq: days}}, function (err, doc){
			        ...
			        if (!doc){
			        	response.send({
				        	"result": "error",
				        	"error": "User has not marked going."
				        });
			        } else {

			        	collection.removeOne({loc: raw_id, user: request.user._id, time: {$eq: days}}, function (err, doc){
			        		if (err){
					            response.send({"result": "error",
					        			       "error": "Internal connection ERROR. Error retrieving site information."});
					        	return;
